...
html_entrepot::html_entrepot():
    ...
    host("Remote host", html_form_input::text, "", "", webdar_css_style::width_100vw_3em),
    port("Port", html_form_input::number, "0", "10", webdar_css_style::width_100vw_3em),
    login("Login", html_form_input::text, "", "", webdar_css_style::width_100vw_3em),
    auth_type("Authentication mode"),
    pass("Password", html_form_input::password, "", "", webdar_css_style::width_100vw_3em),
    auth_from_file("Fetch password from netrc file", html_form_input::check, "", "", ""),
    pub_keyfile("Public key file", "/", "", webdar_css_style::width_100vw, "Select the public key file..."),
    prv_keyfile("Private key file", "/", "", webdar_css_style::width_100vw, "Select the private key file..."),
    knownhosts_check("Check remote host from the known-hosts file", html_form_input::check, "1", "", ""),
    known_hosts_file("Known-hosts file", "/", "", webdar_css_style::width_100vw, "Select the knowhosts file..."),
    wait_time("Network retry delay (s)", html_form_input::number, default_waittime, "5", webdar_css_style::width_100vw_3em),
    verbose("Verbose network connection", html_form_input::check, default_verbose, "", ""),
    ...
    entrep_need_update(false)
{
    ...
    adopt(&auth_type);
    adopt(&pass);
    adopt(&auth_from_file);
    adopt(&pub_keyfile);
    adopt(&prv_keyfile);
    adopt(&knownhosts_check);
    adopt(&known_hosts_file);
    adopt(&wait_time);
    adopt(&verbose);
