...
html_options_read::html_options_read():
    ...
    src_crypto_pass("Passphrase (will be asked later if not provided here)", html_form_input::password, "", "", webdar_css_style::width_60vw),
    src_crypto_size("Cipher block size", 0, "80%"),
    src_ignore_sig_failure("Ignore signature failure", html_form_input::check, "", "", ""),
    src_execute("Command to execute locally before reading each slice", html_form_input::text, "", "", webdar_css_style::width_80vw),
    src_slice_min_digits("Slice minimum digit", html_form_input::number, "0", "6", ""),
    ...
    ref_execute("Command to execute locally before reading each slice", html_form_input::text, "", "", webdar_css_style::width_80vw),
