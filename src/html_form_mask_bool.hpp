...
public:
	...
	/// html_form_mask_expression object to them on this event. This way, the other objects
	/// (mainly from html_mask_form_filename class) can trigger this html_form_mask_expression::update
	...
	/// mandatory call after constructor used to define labels of available object types
    void set_labels_ptr(const std::shared_ptr<const std::deque<std::string> > & lbls);
...
private:
    ...
    std::shared_ptr<const std::deque<std::string> > labels;
    ...
    void push_labels();
    void check_ptr() const { if(!labels) throw WEBDAR_BUG; };
